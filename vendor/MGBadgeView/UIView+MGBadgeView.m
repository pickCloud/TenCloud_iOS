...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if(self = [super initWithFrame:frame]) {
        ...
        _font = [UIFont boldSystemFontOfSize:TCSCALE(10.0)];
        _badgeColor = [UIColor blueColor];
        _textColor = [UIColor whiteColor];
        _outlineColor = [UIColor whiteColor];
        _outlineWidth = 2.0;
        _minDiameter = 15.0;//25.0;
...
- (void)drawRect:(CGRect)rect {
    ...
    if(_badgeValue != 0 || _displayIfZero) {
        ...
        CGRect lblRect = CGRectMake(rect.origin.x, (rect.size.height / 2.0) - (numberSize.height / 2.0)*1.08, rect.size.width, numberSize.height);
...
- (void)mg_updateBadgeViewSize {
    ...
    NSLog(@"update badge width:%.2f,%.2f",badgeWidth,badgeHeight);
