...
@interface TCModifyAppViewController ()<UIImagePickerControllerDelegate,UINavigationControllerDelegate,
...
@implementation TCModifyAppViewController
...
- (void)viewDidLoad {
    ...
    self.descTextView.text = _app.desc;
    /*
    if (IS_iPhoneX)
    {
    ...
    [_sourceButton setTitle:_repoName forState:UIControlStateNormal];
    //NSArray *tagIDArray = _app.labels;
    __weak __typeof(self) weakSelf = self;
    TCTagListRequest *listReq = [TCTagListRequest new];
    [listReq startWithSuccess:^(NSArray<TCEditTag *> *tagArray) {
        for (NSNumber *num in weakSelf.app.labels)
        {
            for (TCEditTag *tg in tagArray)
            {
                if (num.integerValue == tg.tagID)
                {
                    TCEditTag *newTag = [TCEditTag new];
                    newTag.tagID = tg.tagID;
                    newTag.type = tg.type;
                    newTag.name = tg.name;
                    [weakSelf.tagArray addObject:newTag];
                }
            }
        }
